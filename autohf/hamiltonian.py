...
def one_electron_integral_idx(num_elec, atomic_orbitals, idx):
...
def two_electron_integral_idx(num_elec, atomic_orbitals, idx):
...
def one_electron_integral(num_elec, atomic_orbitals):
    """Returns the one electron coefficient for building the Hamiltonian"""
    def one_elec(*args):
        v_fock, w_fock, h_core, eri_tensor = hartree_fock(num_elec, atomic_orbitals)(*args)
        t = jnp.einsum("qr,rs,st->qt", w_fock.T, h_core, w_fock)
        return t
    return one_elec


def two_electron_integral(num_elec, atomic_orbitals):
    """Returns two electron integral"""
    def two_elec(*args):
        v_fock, w_fock, h_core, eri_tensor = hartree_fock(num_elec, atomic_orbitals)(*args)
        t = jnp.swapaxes(jnp.einsum("ab,cd,bdeg,ef,gh->acfh", w_fock.T, w_fock.T, eri_tensor, w_fock, w_fock), 1, 3)
        return t
    return two_elec
